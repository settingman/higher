...
@Component
public class SignalHandler2 extends TextWebSocketHandler {
	@Autowired
	...
	public void afterConnectionEstablished(final WebSocketSession session) {
		...
		sendMessage(session, new WebSocketMessage("Server", MSG_TYPE_JOIN,
					...
					for (Map.Entry<String, WebSocketSession> client : clients.entrySet()) {
						...
						if (!client.getKey().equals(userName)) {
							...
							sendMessage(client.getValue(),
									new WebSocketMessage(userName, message.getType(), data, candidate, sdp));
						}
					}
				}
				break;
