...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.csrf().disable().authorizeRequests().antMatchers("/member/**","/makeup/img/**","/match/img/**").permitAll() // 누구나 mypage 접근허용 - 02/03
		...
		http.formLogin()// Form 로그인 인증 기능이 작동함
				...
				.logout().logoutUrl("/member/logout") // 로그아웃 처리 URL (= form action url)
		...
		http.oauth2Login().successHandler(CustomAuthSuccessHandler);
